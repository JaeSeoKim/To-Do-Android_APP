...
public class EditToDoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ToDoData todo = dbHelper.getToDo(intent.getIntExtra("id",0));
        title = findViewById(R.id.edit_todo_title);
        desc = findViewById(R.id.edit_todo_desc);
        highlight = findViewById(R.id.edit_todo_highlight);

        title.setText(todo.getTitle());
        desc.setText(todo.getDesc());

        FloatingActionButton fab = findViewById(R.id.fab_edit_save_todo);
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (title.getText().toString().trim().equals("")) {
                    ...
                    if (intent != null) {
                        intent.putExtra("title_edited", title.getText().toString());
                        intent.putExtra("desc_edited", desc.getText().toString());
                        intent.putExtra("textColor_edited", color);
                    }
